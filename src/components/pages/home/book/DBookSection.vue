<template>
    <DSection class="pt-24 pb-17.5 flex flex-col gap-12 lg:flex-row lg:items-end lg:gap-40 lg:!px-17.5">
        <div>
            <DSectionHeading>
                Leave your worries at the door and enjoy a 
                <DMarkedWord class="lg:hidden">healthier</DMarkedWord>
                <span class="hidden lg:inline-block">healthier</span>, more 
                <DMarkedWord class="hidden lg:inline-block">precise smile</DMarkedWord>
                <span class="lg:hidden">precise smile</span>
            </DSectionHeading>

            <DDescriptionWrapper>
                We use only the best quality materials on the market in order to provide the best products to our patients, So don’t worry about anything and book yourself.
            </DDescriptionWrapper>

            <DButton>Book an appointment</DButton>
        </div>

        <div class="shrink-0">
            <DPhoto
                src="/pages/home/book.png"
                alt="Book section photo"
            />
        </div>
    </DSection>
</template>